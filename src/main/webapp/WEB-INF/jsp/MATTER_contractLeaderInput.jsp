...
<div class="risktable6">
    ...
    <div class="td_risk  undertake2 textcss"><input type="text" class="large" id="leaderopinions"/></div>
</div>
<div class="btnnew">
    <input type="submit" class="btn btn-primary" value="提交" onclick="contractLeader()"/>
    &nbsp;&nbsp;&nbsp;&nbsp;
    <input type="submit" class="btn btn-primary" value="退回" onclick="contractLeaderBack()"/>
    &nbsp;&nbsp;&nbsp;&nbsp;
</div>
</body>
<script src="/js/jquery.min.js"></script>
<script type="text/javascript">
    function contractLeader() {
        if (!confirm("您确定同意该合同审核吗?")) {
            return
        }
        $.ajax({
            "type": "POST",
            "url": "/api/admin/matter/contractLeader",	//传输路径
            "data": {
                "id":${projectInfo.id},
                "leaderopinions":$('#leaderopinions').val()
            },
    ...
    function contractLeaderBack() {
        if (!confirm("您确定退回该合同吗?")) {
            return
        }
        $.ajax({
            "url": "/api/admin/matter/contractLeaderBack?projectId=${projectInfo.id}",	//传输路径
            "success": function (data) {
                if (data == "OK") {
                    alert("您已退回该合同审核");
                    window.location.href = "/api/admin/iframe/contractLeaderList";
